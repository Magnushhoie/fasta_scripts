...
parser = argparse.ArgumentParser(description="Split FASTA file by sequence lengths",
                                usage=usage)
parser.add_argument("-f", "--fasta", required=True,
                help="Input FASTA to split")
parser.add_argument("-l", "--limits", default="0,800,1200",
                help="FASTA length limits, comma-separated")
...
try:
    ...
    print(f"ERROR: option --limits must be comma separated positive integers (e.g. 0,800,1200)")
...
def filter_fasta_dict_on_seqlen(fasta_dict, min_len=0, max_len=800):
    ...
    log.info(f"Filtering to {len(out_dict)} / {len(fasta_dict)} sequences >= {min_len} and < {max_len}")
...
def main(dict_args):
    ...
    for i, min_len in enumerate(limits):
        ...
        # Write file
        filtered_dict = filter_fasta_dict_on_seqlen(fasta_dict, min_len=min_len, max_len=max_len)
